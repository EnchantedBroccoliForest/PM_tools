 ...
 *   - R1..Rn   for reviewer findings / criticisms (run.criticisms[i].shortId)
 *   - S1..Sn   for sources / evidence records    (run.evidence[i].shortId)
 *   - E1..En   for stage log events              (run.log[i].shortId)
 *
 * IDs are derived from array order and are therefore stable for any given
 * Run JSON: the same Run re-read produces the same ids. The report renderer
 * uses these ids everywhere, and the `--expand` / trace machinery relies on
 * them as lookup keys.
 *
 * This helper is called at production time (end of `orchestrate()`) so the
 * ids land on the canonical Run artifact; the report renderer also calls
 * it idempotently on imported runs that pre-date the field, so legacy
 * artifacts still render with stable references.
 */
...
export function assignShortIds(run) {
  ...
  (run.criticisms || []).forEach((c, i) => {
    if (c && typeof c === 'object') c.shortId = `R${i + 1}`;
  });
